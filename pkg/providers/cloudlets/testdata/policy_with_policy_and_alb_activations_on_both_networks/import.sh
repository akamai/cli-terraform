terraform init
terraform import akamai_cloudlets_application_load_balancer.load_balancer_test_origin test_origin
terraform import akamai_cloudlets_application_load_balancer.load_balancer_test_origin_2 test_origin_2
terraform import akamai_cloudlets_application_load_balancer_activation.load_balancer_activation_test_origin_staging test_origin,staging,2
terraform import akamai_cloudlets_application_load_balancer_activation.load_balancer_activation_test_origin_prod test_origin,prod,2
terraform import akamai_cloudlets_policy.policy test_policy_export
terraform import akamai_cloudlets_policy_activation.policy_activation_prod 2:prod